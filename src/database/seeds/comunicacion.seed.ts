  ...
  public async run(dataSource: DataSource) {
    ...
    const comunicacionEntries = await Promise.all(
      comunicacionData.map(async (comunicacion) => {
        ...
        comunicacionEntry.grupo = (await grupoRepository.findOneBy({
          id_grup: comunicacion.grupo,
        }))!;
        comunicacionEntry.usuario = (await usuarioRepository.findOneBy({
          dni: comunicacion.usuario,
        }))!;
