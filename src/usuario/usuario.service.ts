...
export class UsuarioService {
    ...
    async update(dni: string, dto: UpdateUsuarioDTO): Promise<Usuario | null> {
        const usuario = await this.usuarioRepository.findOne({ where: { dni: dni } });
        if (!usuario) {
            return null;
        }
        ...
        if (dto.estado_cuenta !== undefined) usuario.estado_cuenta = dto.estado_cuenta;
        if (dto.informacion !== undefined) usuario.informacion = dto.informacion;
        if (dto.f_nac !== undefined) usuario.f_nac = new Date(dto.f_nac);
        if (dto.nivel_dependencia !== undefined) usuario.nivel_dependencia = dto.nivel_dependencia;
        if (dto.datos_medicos_dolencias !== undefined) usuario.datos_medicos_dolencias = dto.datos_medicos_dolencias;
        ...
        if (dto.dni !== undefined) usuario.dni = dto.dni ? dto.dni.toUpperCase() : usuario.dni;
