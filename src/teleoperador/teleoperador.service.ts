...
import { CreateTeleoperadorDTO, UpdateTeleoperadorDTO } from './teleoperador.dto';
...
export class TeleoperadorService {
    ...
    async update(id: number, dto: UpdateTeleoperadorDTO): Promise<Teleoperador | null> {
        const teleoperador = await this.teleoperadorRepository.findOne({ where: { id_trab: id } });
        if (!teleoperador) {
            return null;
        }
